...
public class BezierPointEditor : Editor
{
    ...
    void OnSceneGUI()
    {
        ...
        var fmh_103_70_638942768274348031 = point.transform.rotation; Vector3 newPosition = Handles.FreeMoveHandle(point.position, HandleUtility.GetHandleSize(point.position) * 0.2f, Vector3.zero, Handles.CubeHandleCap);
    ...
    private static void HandleConnected(BezierPoint p)
    {
        ...
        var fmh_119_70_638942768274360490 = p.transform.rotation; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
        ...
        var fmh_128_70_638942768274364415 = p.transform.rotation; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
    ...
    private static void HandleBroken(BezierPoint p)
    {
        ...
        var fmh_142_70_638942768274368367 = Quaternion.identity; Vector3 newGlobal1 = Handles.FreeMoveHandle(p.globalHandle1, HandleUtility.GetHandleSize(p.globalHandle1) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
        var fmh_143_70_638942768274371573 = Quaternion.identity; Vector3 newGlobal2 = Handles.FreeMoveHandle(p.globalHandle2, HandleUtility.GetHandleSize(p.globalHandle2) * 0.15f, Vector3.zero, Handles.SphereHandleCap);
